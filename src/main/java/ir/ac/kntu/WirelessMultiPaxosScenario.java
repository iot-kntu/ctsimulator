...
/**
 * Runs Wireless Multi-Paxos on top of the Chaos simulator.
 */
public class WirelessMultiPaxosScenario implements SimulationScenario<ChaosApplication> {
    ...
    public static void main(String[] args) {
        ScenarioRunner.run(new WirelessMultiPaxosScenario());
    }
    ...
    public void configure(NetGraph netGraph, ChaosApplication application) {
        netGraph.getNodes().forEach(node -> {
            ...
            proposedValues.add("CMD_A_" + node.getId());
            proposedValues.add("CMD_B_" + node.getId());
            proposedValues.add("CMD_C_" + node.getId());
            application.setListener(node, new WirelessMultiPaxos(proposedValues));
        });
    }
