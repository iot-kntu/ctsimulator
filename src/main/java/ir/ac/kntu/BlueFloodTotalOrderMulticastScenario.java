...
public class BlueFloodTotalOrderMulticastScenario implements SimulationScenario<BlueFloodApplication> {

    private static final int BLUEFLOOD_REPEAT_SLOTS = 3;
    private static final double LOSS_PROBABILITY = 0.0;
    private static final Logger logger = Logger.getLogger(BlueFloodTotalOrderMulticastScenario.class.getSimpleName());

    private NetGraph netGraph;

    public static void main(String[] args) {
        ScenarioRunner.run(new BlueFloodTotalOrderMulticastScenario());
    }
    ...
    public BlueFloodApplication createApplication(NetGraph netGraph) {
        int rounds = netGraph.getNodeCount() + 2;
    ...
    public void configure(NetGraph netGraph, BlueFloodApplication application) {
        ...
        application.configureScenarioMetadata("BlueFlood Total Order Multicast",
                "ctsimulator",
                "Round-sequenced total order multicast over BlueFlood");

        netGraph.getNodes().forEach(node -> {
            Queue<Object> outbound = new LinkedList<>();
            outbound.add("msg-" + node.getId());
