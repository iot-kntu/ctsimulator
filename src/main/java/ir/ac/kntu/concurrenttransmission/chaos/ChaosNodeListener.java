...
/**
 * New applications must implement this interface. Methods of this interface is
 * called from
...
public interface ChaosNodeListener {
     ...
     * @param context           the simulation context
     * @param packets           simultaneous received packets
     * @param arePacketsSimilar determines are the received packets were similar or
     *                          not. This helps to determine
     *                          the cause of the loss
     */
    void ctPacketsLost(ContextView context, List<FloodPacket<?>> packets, boolean arePacketsSimilar);

    /**
     * Called at the beginning of a new round for each node to create its initial
     * message.
     * The implementation should check if the node is the initiator and act
     * accordingly.
     *
     * @param context   The simulation context.
    ...
    /**
     * Called when a node should initiate the round. In {@link LoyalCtNode}s this
     * method is called once independent
     * of the {@link ChaosTransmissionPolicy#getFloodRepeatCount()}.
     *
     * @param context     the simulation context
     * @param initiator   the initiator node
     * @param whichRepeat determines the n_th repeat of flooding based on
     *                    transmission policy
    ...
    /**
     * gets message for flooding in response to receiving a flood message from a
     * neighbor. In loyal nodes
     ...
     * return no message, a corrupted message or the same message based on
     * traitorous strategy.
     * It is called after
     * {@link ChaosNodeListener#ctPacketsReceived(ContextView, List, FloodPacket, boolean)}
     * method
     * from {@link ir.ac.kntu.concurrenttransmission.blueflood.nodes.FaultyCtNode}
     * and inside its floodMessage method.
     *
     * @param context         the simulation context
